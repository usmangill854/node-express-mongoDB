...
router.put('/:id',async (req,res) => {
    ...
    if(!order){
        return res.status(400).send('cannot created')
    }
    res.send(order);
})
...
console.log(totalPrices)
    ...
    let order = new Order({
        ...
        shippingAddess1:req.body.shippingAddess1,
    ...
    if(!order){
        res.status(400).send('order isnot created')
    }
    res.send(order)
})
...
const totalSales = await Order.aggregate([
    ...
    if(!totalSales){
        return res.status(400).send('ord er sale not generated')
    }
